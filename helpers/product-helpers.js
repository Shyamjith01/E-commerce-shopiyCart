...
module.exports = {
    ...
    topDeals: () => {
        return new Promise(async (resolve, reject) => {
            let topDeals = await db.get().collection(collection.PRODUCT_COLLECTION).find({ 'offer': { $exists: 1 } }).toArray()
            console.log(topDeals, "this are the top Deals");
            resolve(topDeals)
        })
    },
    getOneProduct: (id) => {
